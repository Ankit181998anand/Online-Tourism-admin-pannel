	...
	<div class="col-md-12">
        ...
        if(isset($_FILES['image'])){
          ...
          if ($i_s1==1) {
            echo $i_s1;
                $file_name1;
                $sql="SELECT res_name,states FROM restaurent WHERE res_name = '$name' AND states='$state'";
                $row = $link->query($sql);
                if($row->num_rows>0){
                  ?>
                  <script>
                  setTimeout(function() {
                    swal({
                      title: "Error",
                      text: "Restaurant is already added,for changes please edit it.",
                      type: "error",
                  ...
                  $sql="INSERT INTO `restaurent`(`res_id`,`res_name`, `address`, `states`, `rate`, `image`) VALUES ('$rest_id','$name','$add','$state','$rate','$file_name1')";
