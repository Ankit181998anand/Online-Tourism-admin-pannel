	...
	<div class="col-md-12">
        ...
        if(isset($_FILES['image'])){
          ...
          if ($i_s1==1) {
            echo $i_s1;
                $file_name1;
                $sql="SELECT title,price from destinations where title='$title' AND price='$cost'";
                $row = $link->query($sql);
                if($row->num_rows>0){
                  ?>
                  <script>
                  setTimeout(function() {
                    swal({
                      title: "Error",
                      text: "Destination already added,for changes please edit it.",
                      type: "error",
                  ...
                  $sql="INSERT INTO `destinations`(`title`,`description`, `price`, `image`, `state`) VALUES ('$title','$des', '$cost', '$file_name1','$state')";
